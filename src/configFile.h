// Загрузка данных сохраненных в файл  sConfig.json
String jsonConfig = "{}";
String filePath = "/sConfig.json";
...
bool loadConfig() {
    if(!SPIFFS.exists(filePath)) {
        ...
        saveConfig();   //  Создаем файл записав в него данные по умолчанию
        return false;
    }
    File configFile = SPIFFS.open(filePath, "r");   // Открываем файл для чтения
